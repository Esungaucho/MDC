...
const { requireString, getProjectOr404 } = require('../util');

const STATUSES = ['open', 'answered', 'closed'];

function rfiJson(db, row) {
  const sheet = row.sheet_id
    ? db.prepare('SELECT sheet_number, discipline FROM sheets WHERE id = ?').get(row.sheet_id)
    : null;
  return {
    id: row.id,
...
function resolveSheet(db, projectId, body) {
  if (body.sheetId !== undefined && body.sheetId !== null) {
    const sheet = db.prepare('SELECT * FROM sheets WHERE id = ? AND project_id = ?')
  ...
  if (body.sheetNumber) {
    const sheet = db.prepare('SELECT * FROM sheets WHERE project_id = ? AND sheet_number = ?')
...
function register(app, db) {
  app.get('/api/projects/:id/rfis', ({ params, query }) => {
    ...
    return db.prepare(sql).all(...args).map((r) => rfiJson(db, r));
  });

  app.post('/api/projects/:id/rfis', ({ params, body }) => {
    const project = getProjectOr404(db, params.id);
    const subject = requireString(body, 'subject');
    const sheet = resolveSheet(db, project.id, body);
    ...
    return created(rfiJson(db, db.prepare('SELECT * FROM rfis WHERE id = ?').get(result.lastInsertRowid)));
  });

  app.get('/api/rfis/:id', ({ params }) => {
    const row = db.prepare('SELECT * FROM rfis WHERE id = ?').get(Number(params.id));
    if (!row) throw new ApiError(404, 'rfi_not_found', `No RFI with id ${params.id}`);
    return rfiJson(db, row);
  });

  app.patch('/api/rfis/:id', ({ params, body }) => {
    const row = db.prepare('SELECT * FROM rfis WHERE id = ?').get(Number(params.id));
    if (!row) throw new ApiError(404, 'rfi_not_found', `No RFI with id ${params.id}`);
    const updates = {};
    ...
    db.prepare(`UPDATE rfis SET ${sets} WHERE id = ?`).run(...Object.values(updates), row.id);
    return rfiJson(db, db.prepare('SELECT * FROM rfis WHERE id = ?').get(row.id));
  });
}
